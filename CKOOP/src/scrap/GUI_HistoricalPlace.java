...
public class GUI_HistoricalPlace extends JFrame {

    public GUI_HistoricalPlace() {
        initComponents();
        //addRowToJTable();
    ...
    public static void run() throws IOException {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new GUI_HistoricalPlace().setVisible(true);
                DefaultTableModel model = (DefaultTableModel) jTable1.getModel();
                Object rowData[] = new Object[2];
                for(int i = 0; i < list.size(); i++)
                {
                    rowData[0] = list.get(i).getName();
                    rowData[1] = list.get(i).getPlace();
...
//                    rowData[3] = list.get(i).getHistoryfi();
	...
	private static ArrayList<historicalplace> list;                
}
